...
from app.utils.chat2plot.schema import get_schema_of_chart_config
...
def _requirement_part():
    requirement_part = dedent(
        """
        你需要遵循下面的要求：
        1. 如果用户的作图要求只包含一个维度，请你直接返回配置信息；如果用户的作图要求包含多个维度时，请执行 2；
        2. 请你随机选择其中的一个维度即可，不能出现类似与 "维度A_维度B_..." 的多个维度进行拼接的现象'
        """
    )
